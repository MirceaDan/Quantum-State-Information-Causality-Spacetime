...
## Milestone 4 -- Permutation-Equivariant Quantum Process Memory (added 2026-09-10)

This milestone builds the fix implied by the root-cause analysis above, but as a CLEARLY SEPARATED new module path (`process_topology.py`, `permutation_equivariant_process.py`, `process_memory_observables.py`, `broken_process_control.py`) -- it does not modify `historical_process.py`/`historical_reconstruction.py`/`historical_diagnostics.py`, and it does not yet attempt geometry reconstruction.

**Hidden-permutation-breaking audit (required by the M4 spec).** Every listed risk was checked explicitly:

| Risk | Where it could hide | Finding |
|---|---|---|
| Index-parity rules (`i % 2`) | initial-state construction | Present ONLY in the deliberate `broken_process_control.py` negative control; absent from `simulate_process`, which reads each node's own declared `initial_state_angle`. |
| Fixed adjacency (`(0,1),(1,2),...`) | coupling/relation construction | Relations are explicit `Relation` objects with declared `(first, second, strength, order)`; `permute_process` relabels them, never regenerates them from `range(N-1)`. |
| Pair-generation / enumeration order | observable construction | `pairwise_mutual_information`/`multipartite_information` iterate `itertools.combinations(labels, 2/3)` over the LABEL set; since labels are always `{0,...,N-1}`, this enumeration order is identical before and after any relabeling and never determines a physical quantity, only iteration order for filling an array that is compared by explicit label-indexed gather afterward. |
| Sorting by integer ID | `ProcessTopology.__post_init__` stores `node_labels` sorted; `complete_topology`'s FIRST draft derived relation `order` from `sorted(labels)` enumeration | The stored `node_labels` sort is benign bookkeeping (canonical tensor layout is always `{0,...,N-1}` regardless of permutation, by construction). The `complete_topology` draft bug WAS found and fixed during implementation: `order` is now required as fully explicit caller-supplied data, never derived from `sorted()`/`itertools.combinations()` enumeration -- see `tests/test_process_topology.py::test_complete_topology_requires_every_pair_and_rejects_index_derived_order`. |
| Dict/list insertion order | `node_attributes` dict, `relations` tuple | Relations are always re-sorted by the declared `order` field before execution (`sorted(topology.relations, key=lambda r: r.order)`), never used in raw tuple/dict iteration order. |
| Split assignment | N/A for M4 (no train/test split in this milestone) | Not applicable; M4 does not reconstruct anything, so there is no split to audit yet. |
| Serialization order | JSON export in `run_permutation_equivariant_process_memory.py` | Uses `_to_jsonable` on already label-indexed data structures; serialization order is cosmetic only. |

**Gate result:** `M4_EQUIVARIANCE_PASS=PASS`, `M4_NEGATIVE_CONTROL_PASS=PASS`, `M4_REPRODUCIBILITY_PASS=PASS`, `EXISTING_TEST_SUITE_PASS=PASS` (125 tests: 124 passed, 1 expected xfail, 0 unexpected failures) -> classification **`IMPLEMENTED_AND_VALIDATED`** for the M4 substrate. Per the M4 scientific gate (section 19), this permits proceeding to later milestones that connect this substrate to geometry/causal-topology reconstruction -- but M4 itself makes NO claim about geometry, emergent spacetime, time travel, quantum gravity, or a new physical law. See RESULTS.md for full numbers.

## Milestone 5 -- Hidden Causal Topology Recovery (added 2026-09-10)

Built as a clearly separated new module path (`causal_topology.py`, `topology_generation.py`, `topology_observables.py`, `topology_reconstruction.py`, `topology_identifiability.py`) reusing the M4 substrate directly (no second quantum simulator). Does not modify `historical_process.py`/`historical_reconstruction.py`/`historical_diagnostics.py`/the M4 files.

**A structural bug was found and fixed DURING implementation, not after seeing a convenient result:** the first version of `reconstruct_topology`/`classify_cycle_vs_dag` compared a query's observable bundle against each candidate's bundle using a raw label-indexed distance. This is exactly the section-4 violation the M5 spec warns against ("do not reward the algorithm for guessing arbitrary labels"): a query built from a relabeled topology has no reason to align, entry-by-entry, with a canonically-labeled candidate. The fix, `isomorphism_aware_distance`, minimizes the distance over every relabeling of the candidate (exhaustive permutation search, feasible at M5's small N) before comparing. This is a required correction to match the graph-isomorphism semantics demanded by section 4, not a change made to force a passing test.

**Gate result:** `M4_EQUIVARIANCE_PASS=PASS`, `M5_NEGATIVE_CONTROL_PASS=PASS`, `M5_REPRODUCIBILITY_PASS=PASS`, `M5_NO_LEAKAGE_PASS=PASS`, `EXISTING_TEST_SUITE_PASS=PASS`, but **`M5_LABEL_INVARIANCE_PASS=FAIL`** -> overall gate FAILS -> classification **`IMPLEMENTED_BUT_NOT_FULLY_VALIDATED`**. Per section 27 of the M5 spec, topology-recovery performance numbers are reported (see RESULTS.md) but NOT treated as a validated general conclusion. The strongest currently defensible statements are: (1) cycle-vs-DAG classification is clean (100% accuracy, 0% FPR/FNR) using the causal intervention-response observable; (2) causal/temporal process information (`CAUSAL_PLUS_MEMORY`) substantially outperforms static pairwise correlations (`MI_ONLY`) for exact topology recovery on the mandatory-topology set; (3) label-invariance of that recovery, evaluated per individual query rather than in aggregate, currently fails, and is attributed to finite Monte-Carlo sample size in the candidate-signature estimate rather than to a residual structural confound (M4's equivariance guarantee for the underlying observables is unaffected). Per the milestone's explicit instruction, the process/model was NOT altered to force a passing result; the failure is preserved and documented as the actual finding of this phase.

### M5.1/M5.2 clean-validation correction (2026-09-11)

The previous `M5_LABEL_INVARIANCE_PASS=FAIL` conclusion was based on an invalid protocol, not demonstrated Monte-Carlo instability. The test relabeled the graph but regenerated or misattached node/edge nuisance parameters and selected a fresh schedule. It therefore compared different physical processes. This is now fixed structurally through immutable `PhysicalProcessInstance` and `relabel_physical_process` APIs.

The corrected same-process experiment transforms topology labels, relation endpoints, edge strengths, node initial-state angles, node noise, intervention axes, observable axes, and the exact relation schedule together. Exhaustive N=4 testing gives maximum errors: state `5.42e-17`, pairwise MI `8.88e-16`, multipartite `1.83e-15`, intervention response `1.11e-16`, memory score `5.55e-17`, and global spectrum `4.44e-16`. Thus the structural same-process relabeling gate passes to numerical precision.

Additional hardening: candidate iteration is sorted by `canonical_topology_key`; exact-distance ties are marked ambiguous rather than resolved by input order; `ReferenceLibrary` stores only train-seed bundles and precomputed relabelings; graph precision/recall/F1/edit distance are computed after best isomorphism alignment; and schedule/nuisance validation reuse one frozen process or deliberately resample it, respectively, never conflating the two.

The directionality audit records `gate_is_symmetric=True`, `relation_direction_enters_gate=False`, and `direction_enters_schedule=True`. Any M5 directional recovery claim is therefore about schedule-mediated process direction, not intrinsic direction encoded by partial-SWAP.

**Final clean-validation outcome.** Structural correctness gates pass: same-process relabeling (all node/edge parameters and exact schedule transformed together), reconstruction label invariance, train/test separation, causal-input schema, reproducibility, and prior tests. Empirical gates fail: schedule robustness, nuisance robustness, and exact topology recovery over the complete 34-class N=4 family used here. Classification remains `IMPLEMENTED_BUT_NOT_FULLY_VALIDATED`; no M6 work is justified.

The exact family is the complete set returned by `generate_ensemble(edge_counts=(2,3,4), max_size=1000)`: 34 non-isomorphic directed graphs (1 with two edges, 10 with three edges, 23 with four edges), each using all four nodes. Canonical keys and labelled representatives are serialized in `m5_clean_validation_results.json`; no candidate was removed after observing performance.

The finite-domain pair audit completed all 561 non-isomorphic pairs with 10 independently sampled parameter-pair trials each. `M5_IDENTIFIABILITY_AUDIT_PASS` means the audit completed with the declared domain and records, not that all pairs are separated: 529 pairs were not separated within tolerance 0.05.
